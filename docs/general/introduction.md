...
Once you've done that, this document is intended to give you a taste of how `react-native-boilerplate` works. It still assumes basic knowledge of React, Redux and `react-navigation`. **If you're completely new to React, please refer to https://github.com/petehunt/react-howto instead!**
...
Opening an issue is the fastest way to draw the attention of the team, but please make it a point to read the [docs](https://github.com/keremcubuk/react-native-boilerplate/tree/master/docs) and [contribution instructions](https://github.com/keremcubuk/react-native-boilerplate/blob/master/.github/CONTRIBUTING.md) before you do. The issues section is specifically used for pointing out defects and suggesting enhancements. If you have a question about one of the tools please refer to StackOverflow instead.

## Tech Stack

Here's a curated list of packages that you should be at least familiar with before starting your awesome project. However, the best way to see a complete list of the dependencies is to check [package.json](https://github.com/keremcubuk/react-native-boilerplate/blob/master/package.json).
...
Note that while `react-native-boilerplate` includes a lot of features, many of them are optional and you can find instructions in the docs on how to remove...

- [`redux-saga` or `reselect`](https://github.com/keremcubuk/react-native-boilerplate/blob/master/docs/js/remove.md)
...
- `internals/generators`: This folder has the code to scaffold out new components, containers and routes. Read [more about scaffolding](https://github.com/keremcubuk/react-native-boilerplate/tree/master/docs/general#quick-scaffolding) in the docs.
...
The Redux `store` is the heart of your application. Check out [`configureStore.js`](https://github.com/keremcubuk/react-native-boilerplate/blob/master/app/configureStore.js) to see how we have configured the store.
...
We recommend that you install the relevant IDE extensions for each one of these tools. Once you do, every time you'll press save, all your code will be formatted and reviewed for quality automatically. (Read more at [editor.md](/docs/general/editor).)
...
The react-native-boilerplate building blocks interoperate to produce a seamless application. Let's join these pieces together.
